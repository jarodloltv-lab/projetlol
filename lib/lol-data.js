...
function getDirectChampionMatchupData(champion, opponent, metaByChampion = {}) {
...
export function getChampionMatchupInsights(champion, metaByChampion = {}) {
  ...
  const favorable = combined
    ...
    .sort((first, second) => {
      if (second.positiveSignal !== first.positiveSignal) {
        return Number(second.positiveSignal) - Number(first.positiveSignal);
      }
  ...
  let difficult = combined
    ...
    .sort((first, second) => {
      if (second.negativeSignal !== first.negativeSignal) {
        return Number(second.negativeSignal) - Number(first.negativeSignal);
      }
